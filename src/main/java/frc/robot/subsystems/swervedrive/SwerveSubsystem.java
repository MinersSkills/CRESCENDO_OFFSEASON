...
// Since AutoBuilder is configured, we can use it to build pathfinding commands
  ...
  public void stopAllModules() {
    // Obtém o estado dos módulos swerve (esse método pode variar dependendo da sua implementação).
    SwerveModuleState[] moduleStates = swerveDrive.getStates();
    
    // Define o estado de todos os módulos como velocidade zero.
    for (SwerveModuleState moduleState : moduleStates) {
      moduleState = new SwerveModuleState(0, moduleState.angle); // Define a velocidade como zero.
    }

    // Define os novos estados dos módulos.
    swerveDrive.setModuleStates(moduleStates, true);

    // Se necessário, pare o controle de chassis.
    swerveDrive.setChassisSpeeds(new ChassisSpeeds(0, 0, 0)); // Define a velocidade do chassis como zero.
  }
